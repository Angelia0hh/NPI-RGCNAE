...
res_path = 'result\\NPInter_10412\\combination\\'
def calculate_coordinate(res_path = 'result\\NPInter_10412\\combination\\'):
    path_list = []
    files = []
    ...
    #print(mps)
    return mps
mps = calculate_coordinate('result\\NPInter_10412\\combination\\')
mps2 =  calculate_coordinate('result\\RPI369\\combination\\')
# 所有的方法汇总画图
...
names = ['sort_random_stack_side', 'sort_random_stack_withoutside',
 'sort_random_sum_side', 'sort_random_sum_withoutside',
 'sort_one hot_stack_side', 'sort_one hot_stack_withoutside',
 'sort_one hot_sum_side', 'sort_one hot_sum_withoutside']
...
for label,data in mps.items():
    ...
    if label in names:
        ...
        if label.endswith("stack_withoutside"):
            plt.plot(fpr, tpr,label=label[5:] +' (area = {0:.4f})'.format(mean_auc), lw=1)
...
plt.title('Five-fold mean ROC curve of different feature combinations on NPInter 10412',fontsize=11)
...
plt.title('Five-fold mean ROC curve of different feature combinations on RPI369', fontsize=11)
# plt.legend(fontsize=7,bbox_to_anchor=(1, 0), loc=3, borderaxespad=0)
plt.legend(loc="lower right", fontsize=7)

plt.savefig('pictures/ROC curve of different combinations.svg',bbox_inches='tight')
plt.savefig('pictures/ROC curve of different combinations.png',bbox_inches='tight')
